# FINAL attentive/distracted detection
import torch
...
def apply_emotion_modifiers(engagement, boredom, confusion, frustration, emotion):
    ...
    - happy: engagement+1, boredom-1, confusion+0, frustration-0.5
    - sad: engagement-1, boredom+1, confusion+0.5, frustration+0.5
    - neutral: all unchanged
    - anger: engagement-1, boredom+1, confusion+0.5, frustration+0.5
    
    Any other emotion defaults to neutral (no modifications)
    """
...
def calculate_attentiveness_score(engagement, boredom, confusion, frustration, emotion):
    ...
    Formula:
    Raw_Score = (5 * Engagement_adj) + (3 * (3 - Boredom_adj)) + 
                (1 * (3 - Confusion_adj)) + (1 * (3 - Frustration_adj))
    
    Attentiveness_Score = ((Raw_Score + 9) / 47.5) * 10
...
def predict_attentiveness(behavioral_model, emotion_model, face_cascade, frames, device):
    ...
    Args:
        behavioral_model: Trained AttentionDetectionModel
...
def main():
    """Main function to run continuous attentiveness monitoring"""
